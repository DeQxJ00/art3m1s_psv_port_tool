...
- Video: DAT entries inside PFS archives may contain ordinary data such as font caches, so they always retain their original name and bytes and are never probed or converted. Loose DAT files outside PFS are emitted as same-stem MP4 files at 960×544 using H.264 Main@3.1 and AAC only when a video stream is detected; non-video DAT files remain unchanged. WMV / MP4 / AVI / MPG / MKV use Ratio-based dimension truncation, retain the original codec where encodable, and copy audio; WMV3 becomes WMV2.
